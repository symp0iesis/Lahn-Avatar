...
async def main():
    ...
    # Open a single realtime connection for the conversation
    async with client.beta.realtime.connect(model=DEPLOYMENT_ID) as conn:
        ...
        while True:
            ...
            # 2) Record until user presses Enter
            print("⏺️ Recording... press Enter to stop.")
            frames = []
            def callback(indata, frames_count, time, status):
                frames.append(indata.copy())
            with sd.InputStream(samplerate=INPUT_SAMPLERATE, channels=1,
            ...
            # print(f"Recorded {len(audio_arr)} samples.")
            ...
            # Wait for the server to commit the item
            async for ev in conn:
                if ev.type == "conversation.item.created":
                    break

            # 4) Request assistant response
